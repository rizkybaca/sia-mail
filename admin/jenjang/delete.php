...
if(!isset($_GET['id_jenjang'])){
    ...
    echo '<center><a href="../index.php?page=jenjang-index">click here to back</a></center></div>
    ';
} else {
    include '../../helper/delete_jenjang.php';

if($cek > 0) :

    $result=mysqli_query($conn, "DELETE FROM `jenjang` WHERE `id_jenjang`=$id");
    if ($result) {
