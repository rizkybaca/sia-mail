...
$dataj = mysqli_query($conn,"SELECT `nilai`.*, `pelajaran`.`nama_pelajaran`, `pelajaran`.`kurikulum_id`, `kurikulum`.`nama_kurikulum`, `kelas`.`nama_kelas`, `kelas`.`id_kelas`, `jenjang`.`nama_jenjang`
FROM `nilai`
JOIN `pelajaran`
ON `nilai`.`pelajaran_id`=`pelajaran`.`id_pelajaran`
JOIN `kurikulum`
